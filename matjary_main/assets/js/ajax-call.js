$(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.responseCode == 200) {
                        $("#preloader").hide();
                        swal({title: "Success", text: resp.responseMessage, type: "success"},
                                function () {
    ...
    $("#isThisSubdomainAvailable").click(function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                resp = JSON.parse(resp);
                if (resp.responseCode == 200) {
                    $("#isThisSubdomainAvailableMessage").html('<span style="color:green;">' + resp.responseMessage + '</span>');
                    $("#subDomainAvailSbtBtn").attr('disabled', false);
                } else {
                    $("#isThisSubdomainAvailableMessage").html('<span style="color:red;">' + resp.responseMessage + '</span>');
    ...
    $("#subDomainAvailSbtBtn").click(function () {
        ...
        $.ajax({
            ...
            data: requestData,
            beforeSend: function () {
                $(`<img src='${loader_cir}'>`).appendTo('#isThisSubdomainAvailableMessage');
            },
            success: function (resp) {
                resp = JSON.parse(resp);
                if (resp.responseCode == 200) {
                    ...
                    swal({
                        title: '',
                        text: '<strong>' + sub_domain_name + '</strong> <br/><br/> <span style="color:green;">' + resp.responseMessage + '</span>',
                        type: "info",
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.responseCode == 200) {
                        ...
                        swal({title: "Fail", closeOnClickOutside: false, text: resp.responseMessage, type: "error"},
                                function () {
                                    alert('Please Try different Domain/Sub-Domain Name for Your Store');
        ...
        if (!isvalidate) {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    resp = JSON.parse(resp);
                    if (resp.responseCode == 200) {
                        swal({title: "Success", closeOnClickOutside: false, text: resp.responseMessage, type: "success"});
                    } else {
                        swal({title: "Fail", closeOnClickOutside: false, text: resp.responseMessage, type: "error"});
                    }
                ...
                success: function (resp) {
                    ...
                    setTimeout(function () {
                        if (resp.responseCode == 200) {
                            ...
                            $('#Reset_Pwd_Message').html('<div class="alert alert-success text-center" role="alert">' + resp.responseMessage + '</div>');
                            $('#set_usr_reset_password')[0].reset()
                        } else {
                            $('#Reset_Pwd_Message').html('<div class="alert alert-danger text-center" role="alert">' + resp.responseMessage + '</div>');
                        }
                    }, 2000);
        ...
        if (!isvalidate) {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    resp = JSON.parse(resp);
                    if (resp.responseCode == 200) {
                        $('#newsletter_msgs').html('<div class="alert alert-success text-center" role="alert">' + resp.responseMessage + '</div>');
                        $('#save_newsletter_email_form')[0].reset()
                    } else {
                        $('#newsletter_msgs').html('<div class="alert alert-danger text-center" role="alert">' + resp.responseMessage + '</div>');
                    }
            ...
            if (resp.responseCode == 200) {
                if (view_demo == 'read') {
                    if (template_full_banner == null || template_full_banner == '') {
                        $('.tempDetailsImg_tag').attr('src', base_url + '/uploads/template_half_banners/matjary-default-banner.png');
                    } else {
                        $('.tempDetailsImg_tag').attr('src', base_url + '/uploads/template_half_banners/' + resp.responseData.template_half_banner);
                    }
                    ...
                    if (template_full_banner == null || template_full_banner == '') {
                        $('.templateFullBannerModalIMG').attr('src', base_url + '/uploads/template_full_banners/matjary-default-banner.png');
                    } else {
                        $('.templateFullBannerModalIMG').attr('src', base_url + '/uploads/template_full_banners/' + resp.responseData.template_full_banner);
                    }
                ...
                $('#templateDetailsModalSelectBuyBtn').attr('data-tplprice', template_price);
